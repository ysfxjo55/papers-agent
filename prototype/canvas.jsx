// Canvas — SVG board with timeline, nodes, edges, annotations as marginalia

const {
  CANVAS_W, CANVAS_H, NODE_W, NODE_H, YEAR_MIN, YEAR_MAX,
  getCategoryStyles, LANE_FRACS, REL_LABELS,
  xFromYear, hashId,
} = window.AIMindData;
// Subscribe to theme changes so SVG re-renders with new category palette
...
const CONCEPT_W = 86;
const CONCEPT_H = 28;
const CONCEPT_RADIUS = 105;  // distance from parent center

function conceptPositions(parent, children) {
  // Arrange children in an arc above + below the parent.
  const n = children.length;
  if (n === 0) return [];
  // Spread across ~270deg of an arc, biased downward so they don't collide with annotations
...
// Position annotations around a node — radial slots, deterministic by index
const ANNOT_SLOTS = [
...
function annotationLayout(node, edges, allNodes) {
  const annots = (node.annotations || []).slice(-6);
  if (!annots.length) return [];
  // Determine which sides are "busy" with edges to avoid
  ...
  // Build slots preferring less-busy sides
...
function Canvas({ state, onSelectNode, onClearSelection, selectedId }) {
  ...
  const newSet = React.useMemo(() => new Set(newIds || []), [newIds]);
  ...
  const related = React.useMemo(() => {
    ...
    // include concept children of the active paper
    for (const n of nodes) {
  ...
  const lanes = ["vision", "rl", "foundations", "architecture", "language"];
  ...
  return (
    ...
    >
      ...
      {/* Paper nodes + annotations + concept children */}
      <g>
        {papers.map((node) => {
        ...
        {/* Concept children — only render for active paper */}
...
function Inspector({ node, related, onClose }) {
  useThemeTick();
  if (!node) return null;
  const CATEGORY_STYLES = getCategoryStyles();
  const style = CATEGORY_STYLES[node.category] || CATEGORY_STYLES.other;
...
window.AIMindCanvas = Canvas;
window.AIMindInspector = Inspector;
